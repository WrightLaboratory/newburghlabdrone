...
good_uniq = [518, 519, 522, 523, 535, 538, 553, 554, 555, 556, 557, 558, 560, 561, 562, 563, 564, 565,
 ...
 921, 922, 923, 924, 925, 926, 927, 928, 929, 930, 931, 932, 933, 935, 938]
...
xx = 0
chind = pols[0]
colx = get_slice(beam,beam.V_LC_mean[:,:,0,chind,0],xx,'x')
coly = get_slice(beam,beam.V_LC_mean[:,:,0,chind,0],xx,'y')
fig = plt.figure(figsize=(15,20))
for j, chind in enumerate(pols):
       plt.subplot(5,2,2*j+1)
       for i,p in enumerate(pflights):
          plt.semilogy(beam.y_centers_grid[:,colx,0],flightarr[colx,:,chind,i].T,'o',
             label='Flight '+str(p)+'  x :'+str(beam.x_centers_grid[0,colx,0]))
       ...
       for i,p in enumerate(pflights):
          plt.semilogy(beam.x_centers_grid[coly,:,0],flightarr[:,coly,chind,i].T,'o',
             label='Flight '+str(p)+'  y :'+str(beam.y_centers_grid[coly,0,0]))
          plt.ylim(0.0001,1)
          plt.xlim(-1*sz,sz)
          plt.title('Input: '+str(chind))
       plt.legend(loc='lower center')
