...
def get_maskvals(coordsys):
    ...
    if coordsys=='polar':
        maskin = 6*np.pi/180.
        maskout = 12*np.pi/180 # maskin
    return maskin, maskout

class Beammap_forautoprocessing:
    
    ## Initialize:
    def __init__(self,concatlist=[],gfit=True,ampcorr=True,d0args=[-100,100,5],d1args=[-100,100,5],\
                 coordsys='cartesian',Fargs=[0,1024,512],\
            ...
            for i,chan in enumerate(range(self.n_channels)):
                for j,fr in enumerate(self.faxis):
                    ...
                    self.V_LC_count[:,:,j,i,h]=binned_statistic_2d(x=xf,y=yf,values=valsf,
                                                                statistic='count',
                                                                bins=[self.d0_edges[:,i],self.d1_edges[:,i]]).statistic
