...
file= '/home/erk26/BMX_for_github/thurs_offsets.txt'
with open(file, 'rb') as handle:
    thurs_offsets = pickle.loads(handle.read())

fitsDict = {}
for key in thurs_offsets.keys():
    fitsDict[key] = {}
    for dish in range(4):
        fitsDict[key][dish] = {}
        for freq in range(256):
            try: fitsDict[key][dish][freq] = thurs_offsets[key].get2Dparams(dish,freq)
            except: pass
            if freq < 10: print(freq)
    print('dish '+str(dish)+' done!')

file = '/home/erk26/BMX_for_github/gaussianfits.txt'    
